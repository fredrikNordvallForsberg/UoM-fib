...
and where there is a single atomic predicate, $\qnt$.
...
\begin{definition}
A $\lambda D$-model $(p,Q,G)$ is a $\lambda \forall$-fibration $p:\E\to \B$
...
Indeed an equivalent way to define Abelian group objects
...
We remark that the Abelian group $G$ in a $\lambda D$-model
is analogous to the generic object in a model of System F.

In order to ascertain the value of Definition~\ref{def:uomfib}, we
need to do three things: i) show that a UoM-fibration does in fact provide categorical models of \UoM, ii) give some examples of
UoM-fibrations, and iii) prove some theorems to show the viability of
reasoning at this level of abstraction.\\


\noindent {\bf I. Modelling \UoM in a UoM-fibration:}
\note{Todo: Rewrite to talk about dimensions rather than UoM. Also mention that we need chosen representations of the universal properties.}
To show that \UoMFibrations provide a categorical semantics for \UoM, we must show how to interpret the syntax given in Section~\ref{sec:Not} in any given \UoMFibration. We will use \UoMFibrations to separate the indexing information (the units) from the indexed information (the types and terms). This means, that the base category of the \UoMFibration will be used to interpret unit contexts, and types and terms will be interpreted as objects in the fibres above the unit contexts in which they are defined. Cartesian closure of the fibres will allow us to inductively interpret types built from $\msf(1)$, $\times$ and $\rightarrow$, and we will take the standard approach in categorical logic to interpret quantification of units - by using right adjoints. Finally, since unit expressions for a unit context, are defined as elements of the free Abelian group on that unit context, we will use the Abelian group object structure to interpret such expressions. Formally, we interpret
...
$\bbQ_{\bot}$ to interpret $\qnt$ we obtain a model
corresponding to Kennedy's unit-erasure model.
\end{example}


\begin{example}(Syntax of dimension types)
We can construct a $\lambda D$-model from the syntax.
...
Any given set of physical dimensions (e.g. length, time, speed etc.)
...
Related examples include the relations fibration over $\Set$ and the subobject fibration over $\Set$, and one generalization of the above example is the fibration $\Fam{\C}\to\Set$,
...
and a function $\phi:A \rightarrow B$ such that for any
...
%\begin{example}
%\label{ex:uomLaw}
%Let $G$ be an Abelian group and $\phi$ a $G$-set. Define a fibration $\msf{U_G}:\E \ra \LAb$ to have as fibres $\fibreE{n}$ the functor category $Set^{G^n}$. Then the triple $(U_G, \terminal, \phi)$ is a UoM-fibration. We again defer the proof to Section~\ref{sec:AbSet}.
%\end{example}

% Note that in the $G$-set examples, there is a canonical choice for the $G$-set used to interpret $\qnt$, namely the $G$-set $G \times G \ra G$ (or $\terminal \times \terminal \rightarrow \terminal$ above) given by multiplication in the group. When we consider $G$-sets as functors $G \ra \Set$, this $G$-set is simply the hom-functor which, via Yoneda, makes it a particularly interesting $G$-set to choose. We will expand on this in section~\ref{sec:AbSet}.
...
\begin{theorem}
  Let $(p: \E \ra \B, G, Q)$ be a $\lambda D$-model, $F:\A \ra
  ...
  $\bbracket{F^{\ast}p, G', (G',Q)}$ is a $\lambda D$-model.
\end{theorem}
...
\section{Group Actions and \UoM}\label{sec:AbSet}
...
% \note{****Add to other document that it is easy to generate ``free theorems'' in AbSet fibration and there are many examples especially when playing around with \qnt(u) and arrows}
...
% Though the $\AbSet$ fibration can be used to prove many useful theorems it does not fully capture parametricity. We now construct a more general class of logical relation models that are parametric. For motivation we recall that Reynolds relational semantics arises via a pullback construction as follows.
% \begin{diagram}
...
\begin{theorem}
\label{thm:logUoM}
$(q:\Rel(\E) \rightarrow \LAb, \terminal, Q)$ is a \UoMFibration, where $Q = (\terminal, Q_0, Q_r) \in \Rel(\E)_\terminal$.
\end{theorem}
\begin{proof}
...
%  \end{diagram}
% Where $\blank \times \blank \times \blank  : \C \times \C \times \C \rightarrow \C$ denotes the functor that takes triples $(A,B,C)$ to the object $A\times B \times C$ in $\C$. We define a logical UoM-fibration $p:\E\rightarrow \B$ as the composition of $r$ with the first projection $\pi_1 : \C\times \B\rightarrow \C$, i.e., $p = \pi \circ r$.
%
%
% We now show that this construction does give a \UoMFibration.
...
Suppose that $(u:\B \rightarrow \mathcal{L}, G, Q_0)$ is a UoM-fibration and $p: \E \rightarrow \B$ is a \LamOneFib satisfying the condition on $p$ in Theorem~\ref{thm:CompOfProd}. Choose $Q \in \E_{Q_0}$ and define $q = p \circ u$, then the triple $(q : \E \rightarrow \mathcal{L}, G, Q)$ is a UoM-fibration.
\end{corollary}

\begin{proof}
It is a simple exercise to show that the composition of two \LamOneFibs is a \LamOneFib, and Theorem~\ref{thm:CompOfProd} says that $G$ is an exponentiable object for the composite $q$, and $G$ is Abelian by definition. Hence $(q : \E \rightarrow \mathcal{L}, G, Q)$ is a UoM-fibration.
\end{proof}

With this in mind we can construct a parametric model for \UoM, where both the standard and relational semantics are indexed by units. To this end, let $(u:\B \rightarrow \mathcal{L}, G, Q_0)$ be a UoM-fibration and $p: \E \rightarrow \mathcal{L}$ be a \LamOneFib satisfying the condition on $p$ in Theorem~\ref{thm:CompOfProd}. Then to construct a \emph{parametric UoM-fibration}, we pullback along the composition of the diagonal functor $\Delta : \B \rightarrow \B \times \B$ with the product functor $\blank \times \blank : \B \times \B \rightarrow \B$, and then compose the resulting functor with $u$. Or diagrammatically,
\begin{diagram}
\Rel(\E) \SEpbk		&\rTo 									&\E\\
\dTo<{\Rel(p)}		&									&\dTo>{p} \\
...
\mathcal{L}             		& 									&\mathcal{L}
 \end{diagram}
Let  $Q_r$ be an object of $\E$ in the fibre above $Q_0 \times Q_0$. An object of $\Rel(\E)$ above $A \in \L$ is given by a pair $(B,E)$, where $B$ is an object of $\B$ and $E$ is an object of $\E$ in the fibre above $B \times B$. As expected, the functor $q = u \circ \Rel(p)$ is a UoM-fibration.


\begin{theorem}
\label{thm:ParamUoM}
The triple $(q: \Rel(\E) \rightarrow \mathcal{L}, G, Q)$, where $Q = (Q_0, Q_r)$ is a UoM-fibration.
\end{theorem}
...
\subsection{Using Parametric UoM-Fibrations}
\label{sec:UsingParamUoM}
% We now have two accounts of parametricity for \UoM, one for a unit-erasure semantics and one for a non-unit-erasure semantics. In this section we will put the parametric UoM-fibrations to work to prove a couple of theorems, both of which will apply to the two parametric settings. Firstly, we will we will prove the scaling factor Theorem~\ref{thm:ScalFact} in these settings and then we will show that the type $\forall u_1. \forall u_2. (\qnt(u_1) \rightarrow \qnt(u_2)) \rightarrow \qnt(u_1 \cdot u_2)$ is uninhabited. Since both theorems can be proven, in similar ways, using parametricity for a unit-erasure semantics and using parametricity for a non-unit-erasure semantics, we will prove the first theorem assuming a unit-erasure semantics and the second assuming a non-unit-erasure semantics.
%
%
%
...
% \end{theorem}
% \begin{proof}
% Consider the parametric UoM-fibration as described in Example~\ref{ex:KennedyParam}. Recall that the interpretation of a type $\Delta \vdash \qnt(e) \; \msf{ Type}$ for some unit expression $e$, is given by reindexing $Q$ by the homorphism  $\sem{e}:(\mathbb{Q}^+)^n \rightarrow \mathbb{Q}^+$. Explicitly, this gives
% \[
...
% Units of Measure to be a reflexive graph of UoM-fibrations. This will
...
%
% {\bf Question: I'd like to see this as a UoM-fibration but I cant see
% it.}\label{sec:AbSet}
...
% would complete a reflexive graph of \UoMFibrations
% between Kennedy's relational model and his unit-erasure model - if
% Kennedy's relational model could be cast as a \UoMFibration.



% \section{$G$-Set Parametricity'}
%
% We have seen a model of UoM based upon $G$-sets in the previous

...
Given a group $G$, we write the scripted version, which has morphisms given by elements of $G$ and composition given by group multiplication. Suppose that $\phi: \mathcal{G} \rightarrow \Set$ is a $G$-set
(considered as a functor). Then we will write $|\phi| \coloneqq
...
\[
 |\sem{\forall X. \qnt(X)\rightarrow \qnt(X^n)}| \cong |\sem{\qnt(1^n)}| \cong |\sem{\qnt(1)}| \cong G \text{,}
\]
which implies that terms of type $\forall X. \qnt(X)\rightarrow \qnt(X^n)$ vary only by scalar multiplication.
...
\]
Which is a contradiction since the he left-hand side is equal to $-1$ and the right-hand side is equal to $1$. A similar argument shows that $f(-1)=1$ is also not possible, and hence there exists no such $f$.
\end{proof}
...
. \qnt(\Dvar)$, as such a primitive operation would of course gives rise
...
\begin{theorem}
 \label{thm:AppSubs}
 Suppose that both $\forall X . \qnt(X) \rightarrow \qnt(X) \rightarrow \qnt(1)$ and $\qnt(1)\rightarrow \qnt(1)$, are well-formed types in context. Then
 \[
 |\sem{\forall X . \qnt(X) \rightarrow \qnt(X) \rightarrow \qnt(1)}| \cong |\sem{\qnt(1)\rightarrow \qnt(1)}|
 \]
\end{theorem}
\begin{proof}
We obtain this result as a direct consequence of Theorem~\ref{thm:quantArrowT}.
\end{proof}

We finish this section with another result about a type that is uninhabited, only this time it's higher order.

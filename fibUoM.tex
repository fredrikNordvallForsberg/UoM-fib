...
This paper is about semantic models of programs that manipulate physical quantities. Physical quantities are organized into dimensions, such as $\lengthDim$ or $\timeDim$. A fundamental principle of dimensions is that it is not meaningful to add or compare quantities of different dimension, but they can be multiplied. 

To measure a physical quantity we use units, such as metres for length and seconds for time. We understand these units as chosen constant quantities of a given dimension.
...
%There are also dimensionless constants, e.g. 
%$5:\qnt(1)$.
%,
%and we can multiply quantities, e.g. $5\mathrm{m}:\qnt(\lengthDim)$.
\end{itemize}

...
% Author macros %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{Models of programming languages with physical dimensions}
...
%\begin{proof}
%Let $(p:\E \ra \B, G,\num)$ be a UoM-fibration. The fact that $G$ is an internal Abelian group object %means that there is a finite
...
%$(\terminal,\num)$ lives above $\terminal$ in $F^\ast p$ and hence $F^\ast p$ has the structure of a UoM-fibration with base $\LAb$.
%\end{proof}
...
% \note{****Add to other document that it is easy to generate ``free theorems'' in AbSet fibration and there are many examples especially when playing around with num(u) and arrows}
...
% \item Let $N$ be a fixed object of $\C$ and $P_N$ a chosen object of $\mathcal{P}_{R(B) \times N \times N}$. This is used as the $\num$ object.
% \end{itemize}
%
...
Let $p: \Sub{\Set} \rightarrow \Set$ be the subset fibration. Further, set $\num_0 = \bbQ$, let $R: \LAb \rightarrow \Set$ be the product-preserving functor defined by $R(\terminal) = \bbQ^+$ and $\num_r = \{ (g,a,b) \; | \; ga=b \}$ in the fibre above $\bbQ^+ \times \bbQ \times \bbQ$. The object $\num$ is given by $(\terminal, \num_0, \num_r)$.
...
% We now have two accounts of parametricity for \UoM, one for a unit-erasure semantics and one for a non-unit-erasure semantics. In this section we will put the parametric UoM-fibrations to work to prove a couple of theorems, both of which will apply to the two parametric settings. Firstly, we will we will prove the scaling factor Theorem~\ref{thm:ScalFact} in these settings and then we will show that the type $\forall u_1. \forall u_2. (num(u_1) \rightarrow num(u_2)) \rightarrow num(u_1 \cdot u_2)$ is uninhabited. Since both theorems can be proven, in similar ways, using parametricity for a unit-erasure semantics and using parametricity for a non-unit-erasure semantics, we will prove the first theorem assuming a unit-erasure semantics and the second assuming a non-unit-erasure semantics.
%
%
%
% \begin{theorem}(Scaling Factors Revisited)
% \label{thm:ScalFact2}
% Let $g\in G$ be a  scaling factor. Then for a closed term $\vdash t:\forall u. num(u) \rightarrow num(u^n)$, where $n\in \mathbb{N}$, the following property holds
% \[
% \semo{t} (g \cdot x) = g^n \cdot \semo{t} x
% \]
% \end{theorem}
% \begin{proof}
% Consider the parametric UoM-fibration as described in Example~\ref{ex:KennedyParam}. Recall that the interpretation of a type $\Delta \vdash num(e) \; \msf{ Type}$ for some unit expression $e$, is given by reindexing $\num$ by the homorphism  $\sem{e}:(\mathbb{Q}^+)^n \rightarrow \mathbb{Q}^+$. Explicitly, this gives
% \[
%  \sem{num(e)} = \sem{e}^{\ast}\num = (n,\mathbb{Q}, \sem{e}^\ast P_N)
% \]
% where $\sem{e}^\ast P_N =  \{(g,a,b) \; | \; (\semo{e}g)a = b\}$. To prove the theorem, we notice that since $t$ is a closed term
% \[
% \semo{t:\forall u. num(u) \rightarrow num(u^n)} \times \semo{t:\forall u. num(u) \rightarrow num(u^n)} \in \semr{\forall u. num(u) \rightarrow num(u^n)}
%  \]
% Expanding this, we have that for all $(g,a,b) \in \semr{num(u)}$
% \[
% (g, \semo{t:\forall u. num(u) \rightarrow num(u^n)}a, \semo{t:\forall u. num(u) \rightarrow num(u^n)}b) \in \semr{num(u^n)}
% \]
% Or in other words
% \[
%  \semo{t: \forall u. num(u) \rightarrow num(u^n)} (g \cdot x) = g^n \cdot \semo{t: \forall u. num(u) \rightarrow num(u^n)} x
% \]
% as required.
%
%
...
%
% % \[
% %   \sem{\vdash \forall u. num(u) \rightarrow num(u^n)} = \{ f \in \prod_{g \in G} \sem{\vdash num(u)}g \rightarrow \sem{num(u^n)}g \}
% % \]
% %
% %  \[
% %   \sem{t} \in \sem{\forall u. num(u) \rightarrow num(u^n)}
% %  \]
% % since $\Gamma$ is empty. Expanding this, we have that
% % % \[
% % %  (\semo{t}, \semo{t}) \in \sem{\forall u. num(u) \rightarrow num(u^n)}
% % % \]
% % % which means that
%
% \end{proof}
...
% environments to interpret types of the form $\msf{num}(u)$ as
...
% in the interpretation of $\msf{num}$ as an Abelian universe but allowing it to
...
%   which once more suggests not hard-wiring in the interpretation of $\msf{num}$
% }
%
%
%

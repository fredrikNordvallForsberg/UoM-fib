...
We have studied a typed $\lambda$-calculus with polymorphism over physical dimensions, which we called $\lambda D$ (Sect.~\ref{sec:Not}). We developed a model theory for the calculus. Under the Curry-Howard correspondence, the $\lambda D$-calculus is a fragment of first-order logic where the domain of discourse is an unspecified Abelian group, and so our notion of model~(Def.~\ref{def:lamDModel}) is based on standard fibrational techniques in categorical logic.

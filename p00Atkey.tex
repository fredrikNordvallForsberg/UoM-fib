...
\subparagraph{Dimensions and Dimension Contexts}
...
\subparagraph{Terms and Typing Contexts}
...
the type formers $\unitTy$, $\blank\times\blank$, $\emptyTy$, $\blank+\blank$ and $\blank \! \to \! \blank$ are the usual ones from
...
operation typings $(\mathrm{op} : T_{\mathrm{op}})$ where for each primitive
operation $\mathrm{op} : T_{\mathrm{op}}$, its type $T_{\mathrm{op}}$
is closed (i.e., $\Tj{}{T_{\mathrm{op}}}$). An example set of primitive
operations includes dimension-polymorphic arithmetic and test
...
\section{Categorical Semantics of Dimension Types}
\label{sec:sem}
...
It is well-known that $\lambda \forall$-fibrations give a categorical model of the fragment of first-order logic without existential quantifiers. Nevertheless, we briefly introduce the basic notions now, since they are central to our development. We refer to Jacobs~\cite{jacobs1999categorical} for the full details.
...
Concatenation of dimension contexts will be interpreted using products in the category~$\B$. The reindexing functors  $\pi^\ast:\E_{B}\to \E_{B\times B'}$ for the product projections $\pi:B\times B'\to B$ correspond to context-weakening. A fibration $p:\E\to \B$ is said to have simple products if $\B$ has products and the reindexing functors for the product projections have right adjoints $\forall:E_{B\times B'}\to E_B$ that are compatible with reindexing (`Beck-Chevalley'). 
A fibration is said to have products if this condition holds for all morphisms in the base, not just projections. 
These right adjoints are needed to interpret universal quantification of dimension variables in types.
...
We remark that the object $Q$ in a $\lambda D$-model is analogous to the generic object in a model of System F.
...
\item Dimension expressions $\Dj \Delta D$ are interpreted as morphisms $G^n \to G$ in the base $\B$, by using the structure of the Abelian group object $G$. For example, $\sem{X, Y \vdash X \cdot Y^{-1}} = G\times G\xrightarrow{\id_G\times i}G\times G\xrightarrow m G$.
...
\item Well-formed terms $\Delta; \Gamma \vdash t : T$ are interpreted as morphisms $\sem{t} : \sem{\Gamma} \to \sem{T}$ in the fibre above $\sem{\Delta}$. We assume that there is an interpretation $\sem{\mathrm{op}} : 1 \to \sem{T}$ for each primitive operation $(\mathrm{op} : T) \in \mathrm{Ops}$.
\end{itemize}
...
Care is needed to ensure that the denotational semantics properly respects substitution. This can be done either by requiring that the fibrations are \emph{split}, or by adding explicit coercions to the language~\cite{curien-garner-hofmann}. All the examples of fibrations in this paper are split.
...
Let $\C$ be a bicartesian closed category. Then the functor $\C \to
...
In general, a useful way of building fibrations is by changing the base. If $p:\E\to \B$ is a fibration, and $F:\A\to \B$ is a functor, then the pullback of $p$ along $F$ in $\Cat$, denoted $F^\ast p : F^\ast \E \rightarrow \A$, is again a fibration. The same is true of $\lambda D$-models.
\begin{theorem}
\label{thm:change-of-base}
Let $p:\E\to \B$ be a fibration, and let $F:\A \to \B$ be a functor.
\begin{enumerate}[(i)]
...
Products and coproducts are inherited from $\Set$.
For the function space, let $A$ and $B$ be $G$-sets;
then the set of functions $(A\to B)$ is also a $G$-set,
with action given by $(g\cdot_{(A\to B)}f)(x)\coloneqq g\cdot_B(f(g^{-1}\cdot_A x))$.
...
\label{thm:ScalFact}
Suppose $\Deltadim; \Gammaunits \vdash t : \forall X. \qnt(X) \rightarrow \qnt(X^n)$, where $n\in \mathbb{N}$. Then for all $g \in G$ and $x \in |\sem{\qnt(X)}|$,
...
To answer this question we look at a general method of attaching a (fibrational) logic to a $\lambda D$-model to give a notion of a \emph{relational} $\lambda D$-model. This allows us to reconstruct Kennedy's relational parametricity in our setting (Example~\ref{ex:relKen}), as well as to talk about a relational version of the $\GrpSet$ $\lambda D$-model (Example~\ref{ex:relMon}).
...
\begin{proof}
Clearly $G$ is an Abelian group object in $\mathcal{L}$, and $(Q_0, Q_R)$ is in the fibre $\fibre{(F^\ast \E)}{G}$. To check that $q \circ F^\ast p$ is a bicartesian closed fibration is a simple exercise. Finally, since $p$ has all products, so does $F^\ast p$. Hence, $q \circ F^\ast p$ has simple products by Theorem~\ref{thm:CompOfProd}.
\end{proof}
...
\rightarrow (m, Y)$ by $F(f,g) = (G^f, g, g)$. Finally, let $Q_0
...
\begin{theorem}\label{thm:closed-prog-equiv}
  For any closed term $\vdash t : \bool$, the interpretation of
